...
public final class LocaleManager {
    ...
    public static void prepare() {
        if (loadLocale(GeneralCategory.DEFAULT_LOCALE)) {
            Direct.getInstance().getLogger().info("Loaded default locale");
        } else {
            Direct.getInstance().getLogger().warn("Failed to load default locale");
        }
        
        GeneralCategory generalCategory = Direct.getInstance().getConfig().map(Config::getGeneralCategory).orElseThrow(NullPointerException::new);
        if (!generalCategory.getLocale().equals(GeneralCategory.DEFAULT_LOCALE)) {
            if (loadLocale(generalCategory.getLocale())) {
                Direct.getInstance().getLogger().info("Loaded {} locale", generalCategory.getLocale());
            } else {
                Direct.getInstance().getLogger().warn("Failed to load {} locale", generalCategory.getLocale());
            }
        }
    ...
    public static Component serialize(String key, Object... arguments) {
        ...
        if (matches != arguments.length) {
            Direct.getInstance().getLogger().warn("Incorrect Arguments for {}. Expected {}, got {}", key, matches, arguments.length);
        }
    ...
    private static boolean loadLocale(String name) {
        Map<String, String> locale = deserializeLocaleFile(String.format("/assets/%s/locale/%s.json", Direct.ID, name));
        if (locale != null) {
    ...
    private static Map<String, String> deserializeLocaleFile(String name) {
        ...
        if (inputStream == null) {
            Direct.getInstance().getLogger().warn("Resource {} doesn't exist", name);
            return null;
        }
        ...
        try (Reader reader = new BufferedReader(new InputStreamReader(inputStream))) {
            ...
            Direct.getInstance().getLogger().error("Encountered an error while deserializing {}", name, ex);
            return null;
        }
    }
    ...
    private static String getTranslation(String key) {
        ...
        if (translation == null) {
            Direct.getInstance().getLogger().warn("Missing translation for {}", key);
            return null;
        }
