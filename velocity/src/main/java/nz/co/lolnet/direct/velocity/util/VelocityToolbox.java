...
public class VelocityToolbox {
    ...
    public static String serializePlain(Component component) {
        return PlainComponentSerializer.INSTANCE.serialize(component);
    }
    
    public static Optional<RegisteredServer> getServer(ServerData serverData) {
        if (serverData != null) {
            return VelocityPlugin.getInstance().getProxy().getServer(serverData.getName());
        }
        
        return Optional.empty();
    }
    
    public static Optional<ServerData> getServer(RegisteredServer registeredServer) {
